...
class FSM(BaseModel):
    ...
    def forward(self, x):
        ...
        res = x
        x = conv_block(
            in_features=int(channel_num), out_features=int(channel_num // 8), norm_type=self.norm_type).to(self.device)(x)

        ip = x
        batchsize, channels, dim1, dim2 = ip.shape
        intermediate_dim = int(channels // 2)

        theta = nn.Conv2d(int(channel_num // 8), intermediate_dim, kernel_size=1, padding=0, bias=False).to(self.device)(ip)
        theta = torch.reshape(theta, (batchsize, -1, intermediate_dim))

        phi = nn.Conv2d(int(channel_num // 8), intermediate_dim, kernel_size=1, padding=0, bias=False).to(self.device)(ip)
