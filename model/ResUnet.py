...
from model.MainBlocks import conv_block, ResConv
...
class ResUnet(BaseModel):
    def __init__(self, in_features=3, out_features=3, k=1, norm_type='bn'):
        ...
        self.encode1 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=int(64 * k), out_channels=int(64 * k), kernel_size=3, padding=1))
        self.skip = nn.Conv2d(in_channels=in_features, out_channels=int(64 * k), kernel_size=3, padding=1)
        self.encode2 = ResConv(in_features=int(64 * k), out_features=int(128 * k), stride=2, norm_type=norm_type)
        self.encode3 = ResConv(in_features=int(128 * k), out_features=int(256 * k), stride=2, norm_type=norm_type)
        self.latent = ResConv(in_features=int(256 * k), out_features=int(512 * k), stride=2, norm_type=norm_type)
        self.upsample1 = nn.ConvTranspose2d(in_channels=int(512 * k), out_channels=int(512 * k),
                                            kernel_size=2, stride=2)
        self.decode1 = ResConv(in_features=(int(512 * k) + int(256 * k)), out_features=int(256 * k),
                               norm_type=norm_type)
        self.upsample2 = nn.ConvTranspose2d(in_channels=int(256 * k), out_channels=int(256 * k),
                                            kernel_size=2, stride=2)
        self.decode2 = ResConv(in_features=(int(256 * k) + int(128 * k)), out_features=int(128 * k),
                               norm_type=norm_type)
        self.upsample3 = nn.ConvTranspose2d(in_channels=int(128 * k), out_channels=int(128 * k),
                                            kernel_size=2, stride=2)
